...
export default function Admin({ onBack }) {
  ...
  if (!authed) {
    return <AdminLogin onAuth={(s) => { setSecret(s); setAuthed(true); }} onBack={onBack} />;
  }
  ...
  const renderServiceStatus = (name, status) => {
    ...
    return (
      <div key={name} className="flex items-center gap-2 flex-wrap">
        <span className={`inline-block h-3 w-3 rounded-full shrink-0 ${status === null ? "bg-slate-300 animate-pulse" : ok ? "bg-emerald-500" : "bg-red-500"
          }`} />
